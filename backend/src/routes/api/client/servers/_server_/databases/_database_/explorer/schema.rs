...
mod get {
    ...
    use shared::{
        ApiError, GetState,
        models::{server_database::SchemaTable, user::GetPermissionManager},
    ...
    pub async fn route(
        ...
        if database.database_host.maintenance_enabled {
            return ApiResponse::error(
                "cannot read database schema while database host is in maintenance mode",
            )
            .with_status(StatusCode::EXPECTATION_FAILED)
            .ok();
        }

        let tables = match database.get_schema(&state.database).await {
            Ok(tables) => tables,
