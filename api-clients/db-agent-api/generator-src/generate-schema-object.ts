...
import generateSchemaProperty, { convertType, getSchema, renameSchema } from "@/generate-schema-property"

function resolveAllOf(object: oas31.SchemaObject): oas31.SchemaObject {
    const properties: Record<string, oas31.SchemaObject | oas31.ReferenceObject> = {}

    for (const part of object.allOf!) {
        let schema = part as oas31.SchemaObject
        if (part.$ref) {
            schema = getSchema(part.$ref.split('/').at(-1)!)!
        }
        if (schema.allOf) schema = resolveAllOf(schema)

        Object.assign(properties, schema.properties)
    }

    return { ...object, allOf: undefined, type: 'object', properties }
}
...
export default function generateSchemaObject(output: fs.WriteStream, _spaces: number, parent: string | null, name: string, object: oas31.SchemaObject, inlined: boolean = false) {
    ...
    name = renameSchema(name)

    if (object.allOf) {
        object = resolveAllOf(object)
    }
    ...
    if (object.enum) {
        ...
        if (object.additionalProperties) {
            if (object.propertyNames && !object.propertyNames.$ref && (object.propertyNames as oas31.SchemaObject).format === 'uuid') {
                if (!parent) {
                output.write(`${spaces}pub type ${pascalCase(name)} = IndexMap<uuid::Uuid, ${convertType(object.additionalProperties as any)}>;\n`)
                } else {
                    output.write(`IndexMap<uuid::Uuid, ${convertType(object.additionalProperties as any)}>,\n`)
                }
            } else {
                if (!parent) {
                    output.write(`${spaces}pub type ${pascalCase(name)} = IndexMap<compact_str::CompactString, ${convertType(object.additionalProperties as any)}>;\n`)
                } else {
