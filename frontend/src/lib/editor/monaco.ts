...
import { matchActiveInputShortcut } from '@/plugins/quick-actions/useKeyboardShortcuts.ts';
...
export function forwardGlobalShortcuts(editor: IStandaloneCodeEditor) {
  editor.onKeyDown((event) => {
    const match = matchActiveInputShortcut(event.browserEvent);
    if (!match) return;

    if (match.preventDefault) event.preventDefault();
    event.stopPropagation();
    match.callback(event.browserEvent);
  });
}
