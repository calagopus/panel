...
export interface ExtendableSchema extends ZodType {
  extend(shape: ZodFieldShape): ZodType;
}
...
export function resolveFormValidation(formId: FormId, schema?: ExtendableSchema) {
  ...
  const mergedSchema = schema && Object.keys(zodShape).length ? schema.extend(zodShape) : schema;
...
export function useFormEngine<T extends Record<string, unknown>>(
  ...
  const form = useForm<T>({
    ...formInput,
    initialValues: { ...initialValues, ...(resolved.initialValues as Partial<T>) },
