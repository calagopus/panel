...
execute_build() {
  ...
  for ext_file in /app/extensions/*.c7s.zip; do
    ...
    /app/repo/target/$PROFILE_PATH/panel-rs extensions add "$ext_file" --skip-version-check --accept-license >> "$EXTENSION_LOG" 2>&1
  done
  ...
  if [ $EXIT_CODE -eq 0 ]; then
    ...
    ls -t /app/binaries/*/*/panel-rs 2>/dev/null | tail -n +4 | while read -r old_bin; do
...
if [ -f "$BINARY_PATH" ]; then
  ...
  if [ -z "$LAST_COMPILED" ]; then
    LAST_COMPILED=$(ls -t /app/binaries/*/*/panel-rs 2>/dev/null | head -n 1)
  fi
